...
const PropertyQueries = {
    ...
    getAllProperties: async () => {
        const properties = [];
        const selectPropertiesResult = await pool.query('SELECT * FROM property ORDER BY address ASC');
        for (const propertyRow of selectPropertiesResult.rows) {
            const property = Property.fromRow(propertyRow);
            const leaseQueryResult = await pool.query('SELECT * from lease WHERE property_id = $1 ORDER BY start_date DESC', [propertyRow.id]);
            for (const leaseRow of leaseQueryResult.rows) {
                ...
                const leaseNoteQueryResult = await pool.query('SELECT * FROM lease_note WHERE lease_id = $1 ORDER BY created_at ASC', [lease.id]);
                for (const noteRow of leaseNoteQueryResult.rows) {
                    lease.addLeaseNote(LeaseNote.fromRow(noteRow));
                }
    
                const leaseEventQueryResult = await pool.query('SELECT * FROM lease_event WHERE lease_id = $1 ORDER BY due_date ASC', [lease.id]);
