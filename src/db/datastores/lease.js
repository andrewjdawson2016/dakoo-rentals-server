...
const LeaseQueries = {
    createLease: async (
        ...
        const { previousLeaseId, currentTenants } = isRenewal ? await getRenewalInfo(propertyId) : { previousLeaseId: null, currentTenants: [] };

        try {
            const leaseQueryText = 'INSERT INTO lease(property_id, start_date, end_date, price_per_month, is_renewal, previous_lease_id) VALUES($1, $2, $3, $4, $5, $6) RETURNING id';
            const leaseValues = [propertyId, startDate, endDate, pricePerMonth, isRenewal, previousLeaseId];
            ...
            if (isRenewal) {
                for (let tenant of currentTenants) {
                    const tenantLeaseInsertQuery = 'INSERT INTO tenant_lease (tenant_id, lease_id) VALUES ($1, $2)';
                    await pool.query(tenantLeaseInsertQuery, [tenant.id, leaseId]);
                }
            } else {
                for (let tenant of tenants) {
                    const tenantInsertQuery = 'INSERT INTO tenant (name, email) VALUES ($1, $2) RETURNING id';
                    const tenantInsertResult = await pool.query(tenantInsertQuery, [tenant.name, tenant.email]);
...
const getRenewalInfo = async (propertyId) => {
    ...
    const prevLeaseQuery = 'SELECT id FROM lease WHERE property_id = $1 ORDER BY start_date DESC LIMIT 1';
    ...
    if (result.rows.length > 0) {
        ...
        `;
        const tenantResult = await pool.query(currentTenantQuery, [previousLeaseId]);
